...
const defaultDay = `${YEAR}-${MONTH.padStart(2, 0)}-${DAY.padStart(
				2,
				0
			)}`;
...
const onElDateInputChange = (evt) => {
   ...
   for (let row of filtered) {
      ...
      elNewStudentName.classList.add(
         'js-attendance__table-student-name'
      );
      ...
      elNewStudentName.textContent = getById(
         students,
         row.userId
      ).name;
...
// HTML elements
const elStudentTable = document.querySelector(
   '.attendance__table-student'
);
const elStudentBodyTable = document.querySelector(
   '.attendance__body-student'
);
...
const onElTableBodyClick = (evt) => {
   ...
   if (evt.target.matches('.js-attendance__table-student-name')) {
      ...
      let studentData = students.find(
         (student) => evt.target.textContent === student.name
      );

      // Get students info according
      const studentHistory = attendance.filter(
         (day) => studentData.id === day.id
      );
